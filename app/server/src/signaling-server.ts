...
function handleJoinRoom(ws: WebSocket, roomId: string) {
	...
	console.log(
		`Client joined room ${roomId}. Room size: ${room.clients.size}`
	);
}
...
function leaveRoom(ws: WebSocket, roomId: string) {
	...
	if (room) {
		...
		console.log(
			`Client left room ${roomId}. Room size: ${room.clients.size}`
		);
	}
		...
		if (
			...
			ws.send(
				JSON.stringify({
					type: "error",
					message:
						"AI mode not configured. Please set AZURE_SPEECH_KEY and AZURE_SPEECH_REGION.",
				})
			);
			return;
		}
